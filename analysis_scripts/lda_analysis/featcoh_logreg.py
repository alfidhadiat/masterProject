...
if __name__ == '__main__':
    ...
    base_lr = LogisticRegression(multi_class = 'multinomial', max_iter = 400)
    base_lr.fit(tf_lda, cats)
    base_pred = base_lr.predict_proba(tf_lda)
    base_loss = log_loss(cats, base_pred)
    base_cv_score = np.mean(crossval_linreg(tf_lda, cats))
    ...
    for i in range(100):
        ...
        cur_lr = LogisticRegression(multi_class = 'multinomial', max_iter = 400)
    ...
    loss_df.to_csv('../feature_importances/featcoh_lda_lr.csv')
